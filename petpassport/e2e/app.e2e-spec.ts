...
describe('Starting tests for petpassport', function() {
  ...
  it('website title should be petpassport', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('petpassport');
    })
  });

  it('network-name should be petpassport@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('petpassport@0.0.1.bna');
    });
  });

  it('navbar-brand should be petpassport',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('petpassport');
    });
  });
